...
For predicting the \textit{wine.type} variable, we first considered to apply \textbf{lda}, \textbf{qda} and \textbf{rda}. After that, we tried \textbf{knn}, then \textbf{knn scaling the data set} and finally \textbf{knn after applying lda}. The last method we study is a \textbf{glm} using \textit{binomial family}, in particular, the \textit{logistic regression}. \\ \ \\
...
We have used the original data set without any extracted variables, unless explicitly said so in some sections.
