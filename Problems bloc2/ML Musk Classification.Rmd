...
This method is superior to Leave-One-Out Cross Validation (LOOCV) in that it is computationally less expensive, and given the relatively large amount of datapoints being handled, it preferable to prioritise computational considerations over precision ones. Moreover, James et al. (2014) showed that K-fold CV often leads to more accurate values of the test error than LOOCV.

Below we separate the data into training and learning to evaluate the performance of the models, in an 2:1 training/testing ratio, and after that perform the K-fold CV over the learning data.
...
# Transforming V169 to factor variable to perform cv
pca_learning.data$V169 <- factor(pca_learning.data$V169)

pca_glm_model <- train(form = V169 ~ .,
               data = pca_learning.data[,-1:-2],
               trControl = trainControl(method = "cv", number = K),
               method = "glm",
               family = "binomial")
```
...
simple10_glm_model <- glm(V169 ~ V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12, family = binomial, data = learning.data)
...
simple20_glm_model <- glm(V169 ~ V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12 + V13 + V14 + V15 + V16 + V17 + V18 + V19 + V20 + V21 + V22, family = binomial, data = learning.data)
...
pca_training_data <- princomp(learning.data[-1:-2])
principal_coord <- get_pca_ind(pca_training_data)$coord
reduced_training_data <- cbind(learning.data[169], principal_coord[,1:n])
pca10_glm_model <- glm(V169 ~ Dim.1 + Dim.2 + Dim.3 + Dim.4 + Dim.5 + Dim.6 + Dim.7 + Dim.8 + Dim.9 + Dim.10 + Dim.11 + Dim.12 + Dim.13 + Dim.14 + Dim.15 + Dim.16 + Dim.17 + Dim.18 + Dim.19 + Dim.20, family = binomial, data = reduced_training_data)# + Dim.11 + Dim.12 + Dim.13 + Dim.14 + Dim.15 + Dim.16 + Dim.17 + Dim.18 + Dim.19 + Dim.20, family = binomial, data = reduced_training_data)
...
colnames(pca_test.data)[1:n] <- colnames(reduced_training_data)[-1]
# Validating results
